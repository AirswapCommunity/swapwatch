...
let EthereumTokens = [];
let EthereumTokensByAddress = {};
let EthereumTokensByName = {};
let EthereumTokensBySymbol = {};
...
var getTokenByName = (name) => {
  return EthereumTokensByName[name];
    // let l = 0;
...
var getTokenByAddress = (address) => {
  return EthereumTokensByAddress[address];
    // let l = 0;
...
var setTokens = (tokens) => {
  EthereumTokensByAddress = tokens;
  EthereumTokens = [];
  for (const tokenAddress in tokens) {
    if (tokens[tokenAddress]) {
      const token = tokens[tokenAddress];
      EthereumTokensByName[token.name.toLowerCase()] = token;
      EthereumTokensBySymbol[token.symbol.toLowerCase()] = token;
      EthereumTokens.push(token);
    }
  }
  // sort tokenList
  EthereumTokens.sort((a, b) => {
...
module.exports.EthereumTokens = {
    ...
    ERC20ABI,
    setTokens,
    EthereumTokensByAddress,
    EthereumTokensByName,
    EthereumTokensBySymbol
}