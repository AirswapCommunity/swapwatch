  ...
  componentWillMount() {
    ...
    .then(connected => {
      ...
      if(token1.address !== ETH_address)
        contractToken1 = new web3.eth.Contract(EthereumTokens.ERC20ABI, token1.address);
      if(token2.address !== ETH_address)
        contractToken2 = new web3.eth.Contract(EthereumTokens.ERC20ABI, token2.address);
    ...
    for(let i=0; i<nodeAddressList.length; i++) {
      if(this.state.contractToken1)
        promisesBalance.push(
          ...
          .then((balance) => {
            nodeToken1BalanceList[i] = balance / 10**this.props.token1.decimals;
          })
        )
      else
        promisesBalance.push(
      ...
      if(this.state.contractToken2)
        promisesBalance.push(
          ...
          .then((balance) => {
            nodeToken2BalanceList[i] = balance / 10**this.props.token2.decimals;
          })
        )
      else
        promisesBalance.push(
