...
class CandlestickChart extends Component {
 ...
 d.bb = c;
    ...
    let height = this.state.containerHeight - bottomOffset;

    if (height - 60 < 0) {
      height = 60;
    }

    let bollingerBandElement = this.props.indicator.BollingerBand ? (<BollingerSeries
      yAccessor={(d) => d.bb}
      stroke={bbStroke}
      fill={bbFill} />) : null;

    let emaElements = this.props.indicator.EMA ? (<span>
    ...
    let volumeElement = this.props.indicator.Volume ? (<Chart id={2}
  ...
  render() {
    let chart = (this.props.data && this.props.data.length > 0) ? (
